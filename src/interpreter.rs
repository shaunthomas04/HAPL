use crate::ast::{Expr, Operator, LiteralValue, StaticType, HttpMethod};
...
impl Interpreter {
    ...
    fn eval(&mut self, expr: &Expr) -> ControlFlow {
        match expr {
            ...
            // Server
            // -------------------------
            Expr::ServerDeclaration { port, endpoints } => {
                ...
                'requests: for mut request in server.incoming_requests() {
                    ...
                    let matched = endpoints.iter().find(|e| {
                        e.path == path && match (&method, &e.method) {
                    ...
                    // Run handler in a new scope with params and body injected
                    self.push_scope();
                    ...
                    for stmt in &endpoint.handler {
                        ...
                        // Check if this statement was a Respond
                    ...
                    let (body, content_type) = match &response_value {
                        LiteralValue::Map { .. } => {
                            let json = literal_to_json(response_value.clone())
                                .map(|j| j.to_string())
